...
_RELATIVE_PATH_PAIRS = (
    ("SYSTEM_CLIENTS_RELATIVE_PATH", DEFAULT_CLIENTS_PATH),
    ("SYSTEM_LOGS_RELATIVE_PATH", DEFAULT_LOGS_PATH),
    ("SYSTEM_CACHE_RELATIVE_PATH", DEFAULT_CACHE_PATH),
    ("SYSTEM_HISTORY_RELATIVE_PATH", DEFAULT_HISTORY_PATH),
    ("SYSTEM_UNIVERSE_RELATIVE_PATH", DEFAULT_UNIVERSE_PATH),
)
...
def validate_relative_path_segments(root: Path) -> tuple[PathContractCheck, ...]:
    ...
    for define_name, python_relative in _RELATIVE_PATH_PAIRS:
        ...
        checks.append(
            _check(
                f"relative_{define_name.lower()}",
                f"{define_name} matches Python",
                mql4_relative == expected,
                f"mql4={mql4_relative!r} python={expected!r}",
            )
        )
    return tuple(checks)
...
def validate_config_matches_runtime_root(root: Path) -> PathContractCheck:
    ...
    try:
        ...
        return _check(
            "config_runtime_root",
            "config root_path matches runtime root",
            False,
            str(exc),
        )
    ...
    return _check(
        ...
        "config root_path matches runtime root",
        passed,
        f"config={configured_root} runtime={runtime_root}"
        if passed
        else (
            f"MISMATCH: config.system.root_path={configured_root} "
            f"but run_live.py root={runtime_root}. "
            "Run: python scripts/sync_paths.py"
        ),
    )
...
def validate_mql4_root_matches_config(root: Path) -> PathContractCheck:
    ...
    try:
        ...
        return _check(
            "mql4_config_root",
            "MQL4 compiled root matches config root",
            False,
            str(exc),
        )
    ...
    return _check(
        ...
        "MQL4 compiled root matches config root",
        passed,
        f"both={configured_root}"
        if passed
        else (
            f"MISMATCH: config.system.root_path={configured_root} "
            f"but SYSTEM_ROOT_PATH={compiled_root}. "
            "Run: python scripts/sync_paths.py"
        ),
    )
...
def validate_mt4_data_visible_to_python(root: Path) -> PathContractCheck:
    ...
    try:
        ...
        return _check(
            "mt4_data_visible",
            "Python reads MT4 exports from configured root",
            False,
            str(exc),
        )
    ...
    if not clients_dir.is_dir():
        return _check(
            ...
            "Python reads MT4 exports from configured root",
            False,
            f"clients directory missing at {clients_dir}",
        )
    ...
    if not account_dirs:
        return _check(
            ...
            "Python reads MT4 exports from configured root",
            True,
            f"clients directory exists at {clients_dir} (no account exports yet)",
        )
    ...
    return _check(
        ...
        "Python reads MT4 exports from configured root",
        passed,
        f"found MT4 market exports under {account_dirs[0]}"
        if passed
        else (
            f"no market_*.csv under {account_dirs[0]}. "
            "MT4 EA likely writes to a different root than config.system.root_path"
        ),
    )
...
def validate_no_competing_deployment_trees(root: Path) -> PathContractCheck:
    ...
    if not nested_config.is_file():
        return _check(
            "no_competing_trees",
            "no competing nested SYSTEM deployment tree",
            True,
    ...
    if outer_root == inner_root:
        return _check(
            ...
            "no competing nested SYSTEM deployment tree",
            True,
            f"nested SYSTEM/ config points to same root: {outer_root}",
        )
    ...
    if outer_has_data and inner_has_data:
        return _check(
            ...
            False,
            (
                f"conflicting roots with data: outer={outer_root} inner={inner_root}. "
                "Pick one directory and run scripts/sync_paths.py there only"
            ),
        )
    ...
    return _check(
        ...
        "no competing nested SYSTEM deployment tree",
        True,
        f"nested SYSTEM/ uses different root ({inner_root}) but only one tree has client data",
    )


def run_path_contract_validation(
    ...
    *,
    ...
    checks: list[PathContractCheck] = [
        validate_config_matches_runtime_root(deployment_root),
        validate_mql4_root_matches_config(deployment_root),
        validate_no_competing_deployment_trees(deployment_root),
        *validate_relative_path_segments(deployment_root),
    ]
...
def write_mql4_root_config(root: Path, output_path: Path | None = None) -> Path:
    ...
    content = (
        ...
        "\n"
        "// Auto-generated by scripts/sync_paths.py - do not edit by hand.\n"
        f'#define SYSTEM_ROOT_PATH "{escaped}"\n'
        "\n"
        "#endif\n"
    )
