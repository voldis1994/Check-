...
from engine.deployment.path_contract import (
    ...
    validate_config_matches_runtime_root,
    validate_mql4_root_matches_config,
    validate_no_competing_deployment_trees,
    validate_relative_path_segments,
    write_config_root_path,
    write_mql4_root_config,
)
...
def _install_minimal_deployment(root: Path) -> None:
    ...
    for relative in (
        ...
        "mql4/Include/SYSTEM_RootConfig.mqh",
    ):
...
def test_repo_path_contract_after_sync() -> None:
    root = resolve_deployment_root()
    sync_deployment_paths(root)
...
def test_repo_path_contract_fails_before_sync() -> None:
    ...
    report = run_path_contract_validation(root)
    if report.passed:
        pytest.skip("repository paths already aligned")
