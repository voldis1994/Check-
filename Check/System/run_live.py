...
def _sync_config_root_path(config_path: Path, project_root: Path) -> None:
    ...
    print(f'Updated config system.root_path -> {desired_root}', file=sys.stderr)
...
def _sync_config_instances_from_clients(config_path: Path, project_root: Path) -> None:
    ...
    if not changed:
        return
    config_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False) + '\n', encoding='utf-8')
    print(f'Updated config instances from MT4 data: account_id={first.get('account_id')}, symbol={first.get('symbol')}, magic={first.get('magic')}', file=sys.stderr)
