...
from engine.protocol.constants import AckStatus, AlertLevel, ALL_REASON_CODES, BLOCK_REASON_CODES, CONFIG_SCHEMA_VERSION, Decision, ErrorType, FILE_EXT_CSV, FILE_EXT_JSON, FILE_EXT_JSONL, FILE_EXT_LOG, FILE_EXT_TMP, FILENAME_ACK, FILENAME_CONTROL, FILENAME_DECISION_JOURNAL, FILENAME_ERROR_JOURNAL, FILENAME_INSTANCE_STATE, FILENAME_MARKET, FILENAME_SENSOR, FILENAME_SPREAD_STATE, FILENAME_STATUS, FILENAME_TRADE_JOURNAL, FILENAME_UNIVERSE, FLOAT_TOLERANCE, LogLevel, MARKET_CSV_COLUMNS, MarketRegime, MomentumDirection, NewsImpactLevel, OrderAction, PROTOCOL_SCHEMA_VERSION, REASON_ACCOUNT_NOT_TRADEABLE, REASON_ACK_TIMEOUT, REASON_BOTH_DIRECTIONS_INVALID, REASON_CYCLE_TIMEOUT, REASON_DATA_INVALID, REASON_DUPLICATE_SIGNAL, REASON_EQUAL_SCORES, REASON_EXECUTION_NOT_POSSIBLE, REASON_INSUFFICIENT_DIRECTIONAL_CONFIRMATIONS, REASON_MARKET_QUALITY_TOO_LOW, REASON_MISSING_TAKE_PROFIT, REASON_NEWS_DATA_UNAVAILABLE, REASON_NEWS_WINDOW_ACTIVE, REASON_NO_VALID_SETUP, REASON_POSITION_ALREADY_OPEN, REASON_RISK_DAILY_LOSS, REASON_RISK_MAX_DRAWDOWN, REASON_RISK_MAX_POSITIONS, REASON_SIGNAL_DELTA_TOO_SMALL, REASON_SIGNAL_SCORE_BELOW_MINIMUM, REASON_SPREAD_ABNORMAL, REASON_TRADE_COOLDOWN_ACTIVE, REASON_VOLATILITY_ABNORMAL, RiskResult, SENSOR_CSV_COLUMNS, Side, STATE_SCHEMA_VERSION, StructureBias, SUPPORTED_CONFIG_SCHEMA_VERSIONS, SUPPORTED_PROTOCOL_SCHEMA_VERSIONS, SUPPORTED_STATE_SCHEMA_VERSIONS, SYSTEM_NAME, DEFAULT_ROOT_PATH, TIMEFRAME_M1, TradeEnvironment, TradeEvent, TrendDirection, UNIVERSE_FORBIDDEN_FIELDS, ValidationStatus, WAIT_REASON_CODES, is_block_reason_code, is_supported_config_schema_version, is_supported_protocol_schema_version, is_supported_state_schema_version, is_universe_forbidden_field, is_valid_ack_status, is_valid_decision, is_valid_order_action, is_valid_reason_code, is_valid_risk_result, is_wait_reason_code

def test_system_identity_constants() -> None:
    assert SYSTEM_NAME == 'SYSTEM'
    assert DEFAULT_ROOT_PATH == 'C:\\Check\\System'
...
def test_reason_codes_exist_and_classified() -> None:
    expected_wait = {
        ...
        REASON_POSITION_ALREADY_OPEN,
        REASON_NEWS_DATA_UNAVAILABLE,
    }
    expected_block = {REASON_RISK_MAX_DRAWDOWN, REASON_RISK_DAILY_LOSS, REASON_RISK_MAX_POSITIONS, REASON_SPREAD_ABNORMAL, REASON_VOLATILITY_ABNORMAL, REASON_NEWS_WINDOW_ACTIVE, REASON_ACCOUNT_NOT_TRADEABLE, REASON_DATA_INVALID, REASON_MISSING_TAKE_PROFIT}
...
def test_market_csv_columns() -> None:
    assert MARKET_CSV_COLUMNS == ('time_utc', 'open', 'high', 'low', 'close', 'volume', 'symbol', 'timeframe', 'digits', 'point')

def test_sensor_csv_columns() -> None:
    assert SENSOR_CSV_COLUMNS == ('time_utc', 'bid', 'ask', 'spread', 'spread_points', 'symbol', 'digits', 'point')

def test_universe_forbidden_fields() -> None:
    assert UNIVERSE_FORBIDDEN_FIELDS == frozenset({'signal', 'direction', 'trade', 'buy', 'sell', 'action'})
