...
from engine.protocol.errors import ConfigurationError, DataIOError, ERROR_TYPE_BY_CLASS, ExecutionError, ProtocolError, RiskError, SystemError, ValidationError, get_error_type, wrap_exception
...
def test_system_error_base() -> None:
    ...
    assert error.to_dict() == {'error_type': 'PROTOCOL', 'message': 'base failure', 'module': 'test.module', 'context': {'key': 'value'}}
...
def test_wrap_exception_from_builtin() -> None:
    ...
    wrapped = wrap_exception(original, error_class=DataIOError, module='loader.market_loader', context={'path': '/tmp/x.csv'})
...
def test_all_exception_types_are_subclasses_of_system_error() -> None:
    exception_types = [ProtocolError, ValidationError, DataIOError, ExecutionError, RiskError, ConfigurationError]
