...
from engine.core.timeout import AckTimeoutResult, build_ack_timeout_config, build_ack_timeout_reason, is_ack_timeout_elapsed, log_ack_timeout
...
def test_is_ack_timeout_elapsed_detects_expired_wait() -> None:
    assert is_ack_timeout_elapsed(started_monotonic=100.0, current_monotonic=105.5, ack_timeout_ms=5000)
    assert not is_ack_timeout_elapsed(started_monotonic=100.0, current_monotonic=104.0, ack_timeout_ms=5000)
