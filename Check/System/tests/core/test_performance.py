...
from engine.core.performance import CycleTimingSnapshot, PerformanceState, build_instance_performance_metrics, flush_runtime_performance, format_performance_message, monotonic_elapsed_ms, observe_instance_performance, performance_affects_decisions, read_memory_rss_mb, record_instance_performance, should_emit_performance_log
...
def test_build_instance_performance_metrics_maps_timing_fields() -> None:
    metrics = build_instance_performance_metrics(_instance(), CycleTimingSnapshot(cycle_duration_ms=120, load_duration_ms=30, analysis_duration_ms=40, decision_duration_ms=20, io_wait_ms=30), memory_rss_mb=128.5)
...
def test_format_performance_message_includes_metric_names() -> None:
    metrics = build_instance_performance_metrics(_instance(), CycleTimingSnapshot(cycle_duration_ms=100, load_duration_ms=10, analysis_duration_ms=20, decision_duration_ms=30, io_wait_ms=10), memory_rss_mb=64.0)
...
def test_should_emit_performance_log_respects_interval() -> None:
    ...
    assert should_emit_performance_log(state, metrics_interval_ms=5000, current_monotonic=101.0) is False
    assert should_emit_performance_log(state, metrics_interval_ms=5000, current_monotonic=106.0) is True
...
def test_flush_runtime_performance_writes_metrics_to_system_log(tmp_path: Path) -> None:
    ...
    metrics = build_instance_performance_metrics(instance, CycleTimingSnapshot(cycle_duration_ms=50, load_duration_ms=10, analysis_duration_ms=20, decision_duration_ms=15, io_wait_ms=10))
...
def test_observe_instance_performance_records_pending_metrics(tmp_path: Path) -> None:
    runtime, instance = _startup_runtime(tmp_path)
    state = PerformanceState()
    metrics, updated = observe_instance_performance(runtime, instance, CycleTimingSnapshot(cycle_duration_ms=80, load_duration_ms=12, analysis_duration_ms=25, decision_duration_ms=18, io_wait_ms=12), state=state)
    assert metrics.cycle_duration_ms == 80
    assert len(updated.pending_metrics) == 1

def test_run_instance_cycle_measures_cycle_duration_ms(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    runtime, instance = _startup_runtime(tmp_path)
...
def test_run_runtime_cycles_logs_performance_metrics(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    runtime, instance = _startup_runtime(tmp_path)
    ...
    def _mock_run_execution_engine(**kwargs: object):
        ...
        return ExecutionResult(order_command=order_command, control_published=False, trade_intent_logged=False, ack_interpretation=None, trade_journal_entry=None, state_updated=False)
    monkeypatch.setattr('engine.core.cycle.run_execution_engine', _mock_run_execution_engine)
    run_runtime_cycles(runtime, use_global_universe=False, timestamp_utc=FIXTURE_CYCLE_UTC)
...
def test_performance_metrics_do_not_change_trading_flags(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    runtime, instance = _startup_runtime(tmp_path)
