...
def _status_without_position() -> StatusRecord:
    return StatusRecord(schema_version=PROTOCOL_SCHEMA_VERSION, timestamp_utc='2026-07-07T06:00:00.000Z', account_id='12345', connected=True, trade_allowed=True, balance=10000.0, equity=10020.5, margin_free=9800.0, ea_version='1.0.0', open_positions=())

def _status_with_position(*, ticket: int=555, volume: float=0.1) -> StatusRecord:
    return StatusRecord(schema_version=PROTOCOL_SCHEMA_VERSION, timestamp_utc='2026-07-07T06:00:00.000Z', account_id='12345', connected=True, trade_allowed=True, balance=10000.0, equity=10020.5, margin_free=9800.0, ea_version='1.0.0', open_positions=(StatusPositionSnapshot(symbol='EURUSD', magic=100001, ticket=ticket, side=Side.BUY.value, volume=volume, entry_price=1.1015, stop_loss=1.0988, take_profit=1.1117),))
...
def test_reconcile_position_with_status_syncs_open_position_from_status(tmp_path: Path) -> None:
    ...
    runtime = startup(root_path=tmp_path, config_path=config_path)
    state = runtime.memory.get_or_create(instance).instance_state
    changed = reconcile_position_with_status(runtime.paths, instance, state, _status_with_position(ticket=777, volume=0.2), timestamp_utc='2026-07-07T06:02:00.000Z').changed
...
def test_reconcile_position_with_status_logs_partial_close(tmp_path: Path) -> None:
    ...
    runtime = startup(root_path=tmp_path, config_path=config_path)
    state = runtime.memory.get_or_create(instance).instance_state
    state.update_position(open_ticket=555, position_side=Side.BUY.value, position_volume=0.2, entry_price=1.1015, stop_loss=1.0988, take_profit=1.1117)
    result = reconcile_position_with_status(runtime.paths, instance, state, _status_with_position(ticket=555, volume=0.1), timestamp_utc='2026-07-07T06:02:00.000Z')
    assert result.external_partial_close is True
    assert result.trade_journal_logged is True
    assert state.open_ticket == 555
    assert state.position_volume == 0.1

def test_reconcile_position_does_not_clear_when_other_instance_position_present(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    instance = _instance()
    paths = SystemPaths(tmp_path)
    _install_fixtures(paths, instance)
    ...
    state = runtime.memory.get_or_create(instance).instance_state
    state.update_position(open_ticket=555, position_side=Side.BUY.value, position_volume=0.1, entry_price=1.1015, stop_loss=1.0988, take_profit=1.1117)
    status = StatusRecord(schema_version=PROTOCOL_SCHEMA_VERSION, timestamp_utc='2026-07-07T06:00:00.000Z', account_id='12345', connected=True, trade_allowed=True, balance=10000.0, equity=10020.5, margin_free=9800.0, ea_version='1.0.0', open_positions=(StatusPositionSnapshot(symbol='EURUSD', magic=100001, ticket=555, side=Side.BUY.value, volume=0.1, entry_price=1.1015, stop_loss=1.0988, take_profit=1.1117), StatusPositionSnapshot(symbol='GBPUSD', magic=100002, ticket=888, side=Side.SELL.value, volume=0.05, entry_price=1.25, stop_loss=1.255, take_profit=1.24)))
    result = reconcile_position_with_status(runtime.paths, instance, state, status, timestamp_utc='2026-07-07T06:02:00.000Z')
