...
import pytest
from engine.core.retry import RetryAlertContext, RetryPolicy, build_retry_policy, max_retry_attempts, run_with_retry, validate_control_command_retry
...
def test_run_with_retry_waits_retry_delay_between_attempts() -> None:
    ...
    with pytest.raises(RuntimeError, match='temporary io failure'):
        run_with_retry(policy, _failing_operation, sleep_fn=delays.append)
    assert attempts['count'] == 3
...
def test_validate_control_command_retry_rejects_same_command_id() -> None:
    with pytest.raises(ExecutionError, match='must not be retried with the same command_id'):
        validate_control_command_retry(previous_command_id='cmd-1', command_id='cmd-1')

def test_validate_control_command_retry_allows_new_command_id() -> None:
    validate_control_command_retry(previous_command_id='cmd-1', command_id='cmd-2')
...
def test_run_with_retry_emits_retry_alert_before_final_failure(tmp_path: Path) -> None:
    ...
    with pytest.raises(OSError, match='temporary failure'):
        run_with_retry(policy, _failing_operation, sleep_fn=lambda _: None, alert_context=RetryAlertContext(logger=logger, operation='atomic read'))
