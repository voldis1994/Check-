...
def test_observe_instance_cycle_logs_metrics(tmp_path: Path) -> None:
    ...
    state = observe_instance_cycle(runtime, instance, InstanceCycleResult(instance=instance, timestamp_utc='2026-07-07T06:00:10.000Z', completed=True, error_logged=False), state=MonitoringState())
...
def test_observe_instance_cycle_increments_error_count_on_failure(tmp_path: Path) -> None:
    runtime, instance = _startup_runtime(tmp_path)
    state = observe_instance_cycle(runtime, instance, InstanceCycleResult(instance=instance, timestamp_utc='2026-07-07T06:00:00.000Z', completed=False, error_logged=True))
...
def test_build_instance_metrics_sets_blocked_health_for_block_decision() -> None:
    ...
    instance = _instance()
    decision = DecisionResult(decision_id='decision-1', decision=Decision.BLOCK.value, reason=f'BLOCK: ACCOUNT_NOT_TRADEABLE', preferred_side='NONE', buy_candidate=BuyCandidate(valid=False, invalid_reason='blocked', entry_price=0.0, stop_loss=0.0, take_profit=0.0, component_scores={}, buy_score=0.0), sell_candidate=SellCandidate(valid=False, invalid_reason='blocked', entry_price=0.0, stop_loss=0.0, take_profit=0.0, component_scores={}, sell_score=0.0), buy_score=0.0, sell_score=0.0, analysis_context=AnalysisContext(session='LONDON', regime='trending', news_active=False, context_quality=0.9, trade_environment='FAVORABLE', spread_filter_passed=True))
    metrics = build_instance_metrics(instance, InstanceCycleResult(instance=instance, timestamp_utc='2026-07-07T06:00:10.000Z', completed=True, error_logged=False, decision_result=decision), market_timestamp_utc='2026-07-07T06:00:00.000Z', measured_ack_latency_ms=None, ack_timeout_ms=5000, current_utc='2026-07-07T06:00:10.000Z', error_count=0, error_rate_per_min=0.0)
    assert resolve_instance_health(InstanceCycleResult(instance=instance, timestamp_utc='2026-07-07T06:00:10.000Z', completed=True, error_logged=False, decision_result=decision)) == 'BLOCKED'
...
def test_log_runtime_monitoring_summary_writes_summary_line(tmp_path: Path) -> None:
    ...
    log_runtime_monitoring_summary(runtime, instance_count=2, completed_count=1, failed_count=1, total_errors=1)
    for handler in runtime.system_logger.handlers:
        handler.flush()
    assert 'runtime monitoring summary' in _read_system_log(runtime.paths)

def test_run_runtime_cycles_writes_monitoring_metrics_to_log(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    runtime, instance = _startup_runtime(tmp_path)
