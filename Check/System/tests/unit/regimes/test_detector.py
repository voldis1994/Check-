...
def test_insufficient_history_is_unknown() -> None:
    ...
    bars = make_m15_series(10, drift=0.1)
    snap = det.update(bars)
    assert snap.regime is MarketRegime.UNKNOWN
    assert snap.reason is ReasonCode.HISTORY_INSUFFICIENT


def test_partial_history_still_detects_regime() -> None:
    """With EMA50-worth of bars (but < EMA200), detector must not freeze UNKNOWN forever."""
    base = load_config()
    ...
    det = RegimeDetector(cfg)
    bars = make_m15_series(80, start=50.0, drift=0.25, noise=0.01)
    snap = det.update(bars)
    assert snap.reason is not ReasonCode.HISTORY_INSUFFICIENT
