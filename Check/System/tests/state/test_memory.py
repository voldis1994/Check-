...
def _bar(index: int) -> NormalizedMarketBar:
    return NormalizedMarketBar(time_utc=datetime(2026, 7, 7, 6, index, tzinfo=timezone.utc), open=1.1, high=1.2, low=1.0, close=1.15, volume=100.0, symbol='EURUSD', timeframe='M1', digits=5, point=1e-05, bar_index=index)
...
def test_state_memory_caches_last_analysis_and_decision() -> None:
    ...
    memory = StateMemory(lookback_bars=5)
    instance = Instance(account_id='12345', symbol='EURUSD', magic=100001)
    analysis_context = AnalysisContext(session='LONDON', regime='trending', news_active=False, context_quality=0.9, trade_environment='FAVORABLE', spread_filter_passed=True)
    buy_candidate = BuyCandidate(valid=True, invalid_reason=None, entry_price=1.1, stop_loss=1.09, take_profit=1.12, component_scores={}, buy_score=1.0)
    sell_candidate = SellCandidate(valid=False, invalid_reason='invalid', entry_price=0.0, stop_loss=0.0, take_profit=0.0, component_scores={}, sell_score=0.0)
    decision_result = DecisionResult(decision_id='decision-test-1', decision=Decision.BUY.value, reason='BUY: test', preferred_side=Side.BUY.value, buy_candidate=buy_candidate, sell_candidate=sell_candidate, buy_score=1.0, sell_score=0.0, analysis_context=analysis_context)
    memory.update_analysis_decision(instance, analysis_context=analysis_context, decision_result=decision_result)
    item = memory.get(instance)
    assert item is not None
