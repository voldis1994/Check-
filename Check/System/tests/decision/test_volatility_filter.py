...
def _bar(*, bar_index: int, open_price: float, high: float, low: float, close: float) -> NormalizedMarketBar:
    return NormalizedMarketBar(time_utc=datetime(2026, 1, 1, 0, bar_index, tzinfo=timezone.utc), open=open_price, high=high, low=low, close=close, volume=1.0, symbol='EURUSD', timeframe='M1', digits=5, point=1e-05, bar_index=bar_index)

def test_calculate_relative_volatility_uses_atr_equivalent_over_lookback() -> None:
    bars = (_bar(bar_index=0, open_price=1.1, high=1.101, low=1.099, close=1.1005), _bar(bar_index=1, open_price=1.1005, high=1.101, low=1.0995, close=1.1), _bar(bar_index=2, open_price=1.1, high=1.101, low=1.099, close=1.1005), _bar(bar_index=3, open_price=1.1005, high=1.102, low=1.099, close=1.1015))
...
def test_calculate_relative_volatility_is_near_one_for_uniform_bars() -> None:
    bars = tuple((_bar(bar_index=index, open_price=1.1, high=1.101, low=1.099, close=1.1005) for index in range(5)))
