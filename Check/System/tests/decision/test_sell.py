...
from engine.decision.sell import SellCandidate, build_sell_component_scores, calculate_sell_candidate, calculate_sell_score
...
def _bearish_bars() -> tuple[NormalizedMarketBar, ...]:
    return (_bar(0, 1.104, 1.105, 1.102, 1.1025), _bar(1, 1.1025, 1.103, 1.1005, 1.101), _bar(2, 1.101, 1.1015, 1.0985, 1.099))
...
def _weights() -> dict[str, float]:
    return {'momentum': 1.0, 'trend': 1.0, 'structure': 1.0, 'pressure': 1.0, 'behavior': 1.0, 'impact': 1.0, 'context': 1.0}
...
def _passing_filters() -> tuple:
    return (evaluate_spread_filter(relative_spread=1.0, threshold=1.5), evaluate_volatility_filter(relative_volatility=1.0, threshold=1.5), evaluate_news_filter(_universe(), block_high_impact_news=True))

def _analysis_with_spread_filter(analysis, spread_filter):
    return with_analysis_context(analysis, with_spread_filter_passed(analysis.context, spread_filter.spread_acceptable))
...
def test_build_sell_component_scores_contains_all_components() -> None:
    ...
    assert set(scores) == {'momentum', 'trend', 'structure', 'pressure', 'behavior', 'impact', 'context'}
    assert all((0.0 <= value <= 1.0 for value in scores.values()))

def test_calculate_sell_score_uses_weighted_average() -> None:
    component_scores = {'momentum': 1.0, 'trend': 0.0, 'structure': 0.0, 'pressure': 0.0, 'behavior': 0.0, 'impact': 0.0, 'context': 0.0}
    weights = {key: 0.0 for key in component_scores}
    weights['momentum'] = 1.0
    sell_score = calculate_sell_score(component_scores, weights)
    assert sell_score == 1.0

def test_valid_sell_candidate_contains_all_fields() -> None:
    bars = _bearish_bars()
    analysis = run_analysis_engine(_universe(), bars)
    spread_filter, volatility_filter, news_filter = _passing_filters()
    candidate = calculate_sell_candidate(analysis=_analysis_with_spread_filter(analysis, spread_filter), market_bars=bars, spread_filter=spread_filter, volatility_filter=volatility_filter, news_filter=news_filter, instance_state=_instance_state(), weights=_weights(), stop_loss_buffer=0.0002, reward_ratio=2.0)
...
def test_sell_is_calculated_even_when_buy_is_valid() -> None:
    bars = _bullish_bars()
    analysis = run_analysis_engine(_universe(), bars)
    spread_filter, volatility_filter, news_filter = _passing_filters()
    kwargs = {'analysis': _analysis_with_spread_filter(analysis, spread_filter), 'market_bars': bars, 'spread_filter': spread_filter, 'volatility_filter': volatility_filter, 'news_filter': news_filter, 'instance_state': _instance_state(), 'weights': _weights(), 'stop_loss_buffer': 0.0002, 'reward_ratio': 2.0}
