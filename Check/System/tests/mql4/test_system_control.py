...
def test_system_control_public_functions_are_defined(control_source: str) -> None:
    expected = {'SYSTEM_ResetControlCommand', 'SYSTEM_BuildControlFilePath', 'SYSTEM_IsControlTmpPresent', 'SYSTEM_IsControlReady', 'SYSTEM_ExtractJsonStringField', 'SYSTEM_ExtractJsonToken', 'SYSTEM_ExtractJsonIntField', 'SYSTEM_ExtractJsonDoubleField', 'SYSTEM_IsSupportedOrderAction', 'SYSTEM_ControlRequiresOrderExecution', 'SYSTEM_ParseControlCommand', 'SYSTEM_ValidateControlInstance', 'SYSTEM_ReadControlCommand', 'SYSTEM_ControlPerformsAnalysis'}
...
def test_system_build_control_file_path_uses_instance_template() -> None:
    path = control_reference.build_control_file_path('C:\\Check\\System', '12345', 'EURUSD', 100001)
    assert path == 'C:\\Check\\System\\data\\clients\\12345\\control_EURUSD_100001.json'
...
def test_system_is_control_ready_requires_final_file_without_tmp() -> None:
    assert control_reference.is_control_ready('path', tmp_exists=False, file_exists=True)
    assert not control_reference.is_control_ready('path', tmp_exists=True, file_exists=True)
    assert not control_reference.is_control_ready('path', tmp_exists=False, file_exists=False)
...
def test_wrong_magic_is_rejected() -> None:
    command, error = control_reference.read_control_command(CONTROL_JSON_VALID, account_id='12345', symbol='EURUSD', magic=999999)
    assert command is None
    assert 'magic' in error

def test_wrong_symbol_is_rejected() -> None:
    command, error = control_reference.read_control_command(CONTROL_JSON_VALID, account_id='12345', symbol='GBPUSD', magic=100001)
    assert command is None
...
def test_none_action_is_parsed_without_order_execution() -> None:
    command, error = control_reference.read_control_command(CONTROL_JSON_NONE, account_id='12345', symbol='EURUSD', magic=100001)
    assert error == ''
    assert command is not None
...
def test_system_read_control_command_rejects_tmp_file() -> None:
    command, error = control_reference.read_control_command(CONTROL_JSON_VALID, account_id='12345', symbol='EURUSD', magic=100001, tmp_exists=True)
    assert command is None
...
def test_valid_control_read_from_published_python_control_file(tmp_path) -> None:
    ...
    command, error = control_reference.read_control_command(raw_text, account_id='12345', symbol='EURUSD', magic=100001)
    assert error == ''
    assert command is not None
