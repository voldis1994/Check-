...
import pytest
from engine.core.paths import DEFAULT_CACHE_PATH, DEFAULT_CLIENTS_PATH, DEFAULT_HISTORY_PATH, DEFAULT_LOGS_PATH, DEFAULT_UNIVERSE_PATH
...
def test_system_paths_source_defines_root_path_as_c_system(root_config_source: str) -> None:
    assert mql_source.parse_define(root_config_source, 'SYSTEM_ROOT_PATH') == DEFAULT_ROOT_PATH
...
def test_system_paths_public_functions_are_defined(paths_source: str) -> None:
    expected = {'SYSTEM_GetRootPath', 'SYSTEM_GetClientsRelativePath', 'SYSTEM_GetLogsRelativePath', 'SYSTEM_GetCacheRelativePath', 'SYSTEM_GetHistoryRelativePath', 'SYSTEM_GetUniverseRelativePath', 'SYSTEM_JoinPath', 'SYSTEM_BuildClientsDir', 'SYSTEM_BuildAccountDir', 'SYSTEM_BuildAccountJournalDir', 'SYSTEM_BuildAccountStateDir', 'SYSTEM_BuildLogsDir', 'SYSTEM_BuildCacheDir', 'SYSTEM_BuildHistoryDir', 'SYSTEM_BuildUniverseDir', 'SYSTEM_DirectoryExists', 'SYSTEM_EnsureDirectory', 'SYSTEM_EnsureDirectories', 'SYSTEM_EnsureAccountDirectories', 'SYSTEM_InitPaths'}
...
def test_system_join_path_normalizes_separators() -> None:
    assert path_reference.join_path('C:\\Check\\System', 'data\\clients') == 'C:\\Check\\System\\data\\clients'
    assert path_reference.join_path('C:\\Check\\System\\', '\\data\\clients') == 'C:\\Check\\System\\data\\clients'
    assert path_reference.join_path('', 'data\\clients') == 'data\\clients'
    assert path_reference.join_path('C:\\Check\\System', '') == 'C:\\Check\\System'
...
def test_system_build_account_journal_dir_matches_python_paths() -> None:
    root = DEFAULT_ROOT_PATH
    clients = DEFAULT_CLIENTS_PATH.replace('/', '\\')
    expected = path_reference.join_path(path_reference.build_account_dir(root, clients, '12345'), 'journal')
...
def test_system_build_account_state_dir_matches_python_paths() -> None:
    root = DEFAULT_ROOT_PATH
    clients = DEFAULT_CLIENTS_PATH.replace('/', '\\')
    expected = path_reference.join_path(path_reference.build_account_dir(root, clients, '12345'), 'state')
...
def test_system_ensure_account_directories_creates_account_journal_and_state(tmp_path: Path, paths_source: str) -> None:
    ...
    account_dir = path_reference.ensure_account_directories(tmp_path, '12345', clients='data\\clients', logs='data\\logs', cache='data\\cache', history='data\\history', universe='data\\universe')
...
def test_ea_startup_creates_data_clients_account_directory(tmp_path: Path, paths_source: str) -> None:
    account_id = '67890'
    account_dir = path_reference.ensure_account_directories(tmp_path, account_id, clients=mql_source.parse_define(paths_source, 'SYSTEM_CLIENTS_RELATIVE_PATH'), logs=mql_source.parse_define(paths_source, 'SYSTEM_LOGS_RELATIVE_PATH'), cache=mql_source.parse_define(paths_source, 'SYSTEM_CACHE_RELATIVE_PATH'), history=mql_source.parse_define(paths_source, 'SYSTEM_HISTORY_RELATIVE_PATH'), universe=mql_source.parse_define(paths_source, 'SYSTEM_UNIVERSE_RELATIVE_PATH'))
...
def test_system_path_points_to_c_system_root(root_config_source: str, paths_source: str) -> None:
    ...
    assert account_dir == f'{DEFAULT_ROOT_PATH}\\data\\clients\\12345'
