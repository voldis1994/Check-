...
def public_function_names(source: str) -> list[str]:
    pattern = re.compile('^(?:void|bool|string|int|double|datetime)\\s+(SYSTEM_[A-Za-z0-9_]+)\\s*\\(', re.MULTILINE)
    return pattern.findall(source)

def function_body(source: str, name: str) -> str:
    match = re.search(f'(?:void|bool|string|int|double|datetime)\\s+{re.escape(name)}\\s*\\([^{{]*\\)\\s*\\{{', source)
    if match is None:
