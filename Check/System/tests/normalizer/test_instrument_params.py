...
import pytest
from engine.normalizer.instrument_params import InstrumentParams, calculate_pip, derive_instrument_params, detect_params_change
...
def test_calculate_pip_for_five_digits_uses_point_times_ten() -> None:
    assert calculate_pip(point=1e-05, digits=5) == 0.0001
...
def test_derive_instrument_params_from_first_valid_market_row() -> None:
    ...
    assert params == InstrumentParams(symbol='EURUSD', digits=5, point=1e-05, pip=0.0001)
...
def test_detect_params_change_when_digits_or_point_changed() -> None:
    current = InstrumentParams(symbol='EURUSD', digits=5, point=1e-05, pip=0.0001)
    ...
    incoming_same = InstrumentParams(symbol='EURUSD', digits=5, point=1e-05, pip=0.0001)
