...
from engine.journal.trade_journal import INTENT_REASON_PREFIX, TradeIntentParams, append_trade_journal_entry, build_trade_ack_entry, build_trade_intent_entry, build_trade_journal_path, log_trade_ack, log_trade_intent, update_trade_journal_ack
...
def _open_intent_params() -> TradeIntentParams:
    return TradeIntentParams(trade_id='trade-open-1', command_id='cmd-open-1', event=TradeEvent.OPEN.value, reason='OPEN BUY after risk allow', side=Side.BUY.value, volume=0.1, price=1.1031)

def _modify_intent_params() -> TradeIntentParams:
    return TradeIntentParams(trade_id='trade-modify-1', command_id='cmd-modify-1', event=TradeEvent.MODIFY.value, reason='TRADE_MANAGEMENT_TRAILING: stop loss raised', ticket=1001, price=1.1035)

def _close_intent_params() -> TradeIntentParams:
    return TradeIntentParams(trade_id='trade-close-1', command_id='cmd-close-1', event=TradeEvent.CLOSE.value, reason='TRADE_MANAGEMENT_TIME_STOP: maximum bars reached', ticket=1001, volume=0.1)

def _ack_record(*, command_id: str, status: str=AckStatus.SUCCESS.value, ticket: int | None=1001) -> AckRecord:
    return AckRecord(schema_version='1.0.0', timestamp_utc='2026-07-07T06:01:00.000Z', command_id=command_id, account_id='12345', symbol='EURUSD', magic=100001, status=status, ticket=ticket)
...
def test_build_trade_intent_entry_maps_open_intent_fields() -> None:
    entry = build_trade_intent_entry(_instance(), _open_intent_params(), timestamp_utc='2026-07-07T06:00:00.000Z')
...
def test_build_trade_ack_entry_applies_ack_result() -> None:
    intent_entry = build_trade_intent_entry(_instance(), _open_intent_params(), timestamp_utc='2026-07-07T06:00:00.000Z')
    ack_entry = build_trade_ack_entry(intent_entry, _ack_record(command_id='cmd-open-1', status=AckStatus.SUCCESS.value, ticket=555), timestamp_utc='2026-07-07T06:01:00.000Z', price=1.10315)
...
def test_trade_journal_entry_contains_all_section_19_9_fields() -> None:
    ...
    assert data['price'] == pytest.approx(1.1031)

def test_log_trade_intent_writes_open_intent_before_ack(tmp_path) -> None:
    paths = SystemPaths(root_path=tmp_path)
    instance = _instance()
    entry = log_trade_intent(paths, instance, _open_intent_params(), timestamp_utc='2026-07-07T06:00:00.000Z')
...
def test_log_trade_ack_updates_entry_with_ack_result(tmp_path) -> None:
    paths = SystemPaths(root_path=tmp_path)
    instance = _instance()
    log_trade_intent(paths, instance, _open_intent_params(), timestamp_utc='2026-07-07T06:00:00.000Z')
    updated = log_trade_ack(paths, instance, _ack_record(command_id='cmd-open-1', status=AckStatus.SUCCESS.value, ticket=555), timestamp_utc='2026-07-07T06:01:00.000Z', price=1.10315)
    journal_text = build_trade_journal_path(paths, instance).read_text(encoding='utf-8')
...
def test_update_trade_journal_ack_rejects_missing_command_id(tmp_path) -> None:
    ...
    with pytest.raises(Exception, match='trade journal entry not found'):
        update_trade_journal_ack(paths, instance, _ack_record(command_id='cmd-missing', status=AckStatus.FAILED.value), timestamp_utc='2026-07-07T06:01:00.000Z')

def test_append_trade_journal_entry_is_append_only_for_multiple_intents(tmp_path) -> None:
    paths = SystemPaths(root_path=tmp_path)
    instance = _instance()
    first = build_trade_intent_entry(instance, _open_intent_params(), timestamp_utc='2026-07-07T06:00:00.000Z')
    second = build_trade_intent_entry(instance, _modify_intent_params(), timestamp_utc='2026-07-07T06:02:00.000Z')
...
def test_log_trade_intent_supports_modify_and_close_events(tmp_path) -> None:
    paths = SystemPaths(root_path=tmp_path)
    instance = _instance()
    modify_entry = log_trade_intent(paths, instance, _modify_intent_params(), timestamp_utc='2026-07-07T06:02:00.000Z')
    close_entry = log_trade_intent(paths, instance, _close_intent_params(), timestamp_utc='2026-07-07T06:03:00.000Z')
...
def test_trade_journal_is_isolated_by_instance(tmp_path) -> None:
    ...
    log_trade_intent(paths, instance_a, _open_intent_params(), timestamp_utc='2026-07-07T06:00:00.000Z')
    log_trade_intent(paths, instance_b, TradeIntentParams(trade_id='trade-gbp-1', command_id='cmd-gbp-1', event=TradeEvent.OPEN.value, reason='OPEN SELL after risk allow', side=Side.SELL.value, volume=0.2), timestamp_utc='2026-07-07T06:00:00.000Z')
...
def test_log_trade_ack_records_failed_ack_status(tmp_path) -> None:
    ...
    instance = _instance()
    log_trade_intent(paths, instance, _open_intent_params(), timestamp_utc='2026-07-07T06:00:00.000Z')
    updated = log_trade_ack(paths, instance, _ack_record(command_id='cmd-open-1', status=AckStatus.FAILED.value, ticket=None), timestamp_utc='2026-07-07T06:01:00.000Z')
