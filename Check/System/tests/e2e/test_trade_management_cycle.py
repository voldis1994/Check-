...
def _startup_runtime_for_trade_management(tmp_path: Path, *, trade_management_overrides: dict[str, object] | None=None):
...
def test_e2e_open_modify_cycle_updates_stop_loss(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    simulator.export_tick(instance, market_scenario='bullish', timestamp_utc=modify_timestamp, close_override=1.108)
    modify_result = run_instance_cycle(runtime, instance, use_global_universe=False, timestamp_utc=modify_timestamp)
...
def test_e2e_open_close_cycle_clears_position(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _patch_fixed_command_id(monkeypatch)
    paths = SystemPaths(tmp_path)
    simulator = MT4Simulator(paths)
    instance = _instance()
    ...
    close_result = run_instance_cycle(runtime, instance, use_global_universe=False, timestamp_utc=close_timestamp)
...
def test_e2e_open_partial_close_cycle_reduces_volume(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _patch_fixed_command_id(monkeypatch)
    paths = SystemPaths(tmp_path)
    simulator = MT4Simulator(paths)
    instance = _instance()
    ...
    assert partial_result.execution_result.order_command.volume == pytest.approx(original_volume * 0.5, abs=0.01)
