...
def _status(*, balance: float=10000.0, equity: float=10000.0) -> StatusRecord:
    return StatusRecord(schema_version='1.0.0', timestamp_utc='2026-07-07T06:00:00.000Z', account_id='12345', connected=True, trade_allowed=True, balance=balance, equity=equity, margin_free=9000.0, ea_version='1.0.0')

def _instance_state(*, day_start_balance: float | None=None, peak_equity: float | None=None) -> InstanceState:
...
def test_build_risk_context_reads_persisted_instance_state_metrics() -> None:
    context = build_risk_context(status=_status(balance=10000.0, equity=9500.0), instance_state=_instance_state(day_start_balance=10000.0, peak_equity=10000.0))
...
def test_build_risk_context_defaults_to_zero_without_persisted_metrics() -> None:
    context = build_risk_context(status=_status(balance=10000.0, equity=9000.0), instance_state=_instance_state())
