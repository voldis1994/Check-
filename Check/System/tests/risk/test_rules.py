...
from engine.protocol.constants import REASON_ACCOUNT_NOT_TRADEABLE, REASON_RISK_DAILY_LOSS, REASON_RISK_MAX_DRAWDOWN, REASON_RISK_MAX_POSITIONS
from engine.protocol.models import RiskConfig, StatusRecord
from engine.risk.rules import RiskContext, RiskRuleResult, check_max_daily_loss, check_max_drawdown, check_max_open_positions, check_trade_allowed, evaluate_risk_rules, open_position_count
...
def _risk_context(*, daily_loss_percent: float=0.0, drawdown_percent: float=0.0) -> RiskContext:
    return RiskContext(daily_loss_percent=daily_loss_percent, drawdown_percent=drawdown_percent)
...
def test_evaluate_risk_rules_returns_first_block_reason() -> None:
    result = evaluate_risk_rules(status=_status(trade_allowed=False), instance_state=_instance_state(with_position=True), risk_config=_risk_config(), risk_context=_risk_context(daily_loss_percent=5.0, drawdown_percent=15.0))
...
def test_evaluate_risk_rules_allows_when_all_checks_pass() -> None:
    result = evaluate_risk_rules(status=_status(trade_allowed=True), instance_state=_instance_state(with_position=False), risk_config=_risk_config(), risk_context=_risk_context(daily_loss_percent=0.5, drawdown_percent=3.0))
    assert result.allowed
    assert result.reason is None

def test_every_blocked_rule_includes_reason() -> None:
    blocked_results = [check_trade_allowed(status=_status(trade_allowed=False)), check_max_open_positions(instance_state=_instance_state(with_position=True), risk_config=_risk_config()), check_max_daily_loss(risk_config=_risk_config(), daily_loss_percent=3.0), check_max_drawdown(risk_config=_risk_config(), drawdown_percent=12.0)]
