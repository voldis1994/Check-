...
from engine.execution.control_writer import build_control_command, build_control_path, publish_control, write_control_file
...
def _open_order_command() -> OrderCommand:
    return OrderCommand(command_id='cmd-open-1', action=OrderAction.OPEN.value, reason='BUY: preferred side selected', decision_id='decision-123', side=Side.BUY.value, volume=0.1, stop_loss=1.0988, take_profit=1.1117)

def _none_order_command() -> OrderCommand:
    return OrderCommand(command_id='cmd-none-1', action=OrderAction.NONE.value, reason='WAIT: equal scores', decision_id='decision-456')
...
def test_build_control_command_maps_order_command_and_instance_fields() -> None:
    ...
    control_command = build_control_command(instance, order_command, timestamp_utc='2026-07-07T06:00:00.000Z')
    ...
    assert control_command.stop_loss == pytest.approx(1.0988)
    assert control_command.take_profit == pytest.approx(1.1117)
...
def test_control_output_contains_all_section_19_4_fields(tmp_path) -> None:
    paths = SystemPaths(root_path=tmp_path)
    instance = _instance()
    control_command = publish_control(paths, instance, _open_order_command(), timestamp_utc='2026-07-07T06:00:00.000Z')
...
def test_write_control_file_uses_atomic_tmp_and_rename(tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
    paths = SystemPaths(root_path=tmp_path)
    instance = _instance()
    control_command = build_control_command(instance, _open_order_command(), timestamp_utc='2026-07-07T06:00:00.000Z')
...
def test_write_control_file_rejects_instance_mismatch(tmp_path) -> None:
    paths = SystemPaths(root_path=tmp_path)
    instance = _instance()
    mismatched = ControlCommand(schema_version=PROTOCOL_SCHEMA_VERSION, timestamp_utc='2026-07-07T06:00:00.000Z', command_id='cmd-open-1', account_id='12345', symbol='GBPUSD', magic=100001, action=OrderAction.OPEN.value, reason='BUY selected', decision_id='decision-123', side=Side.BUY.value, volume=0.1)
...
def test_publish_control_writes_parseable_control_json(tmp_path) -> None:
    paths = SystemPaths(root_path=tmp_path)
    instance = _instance()
    control_command = publish_control(paths, instance, _none_order_command(), timestamp_utc='2026-07-07T06:00:00.000Z')
...
def test_publish_control_is_isolated_by_instance(tmp_path) -> None:
    ...
    publish_control(paths, instance_a, _open_order_command(), timestamp_utc='2026-07-07T06:00:00.000Z')
    publish_control(paths, instance_b, OrderCommand(command_id='cmd-gbp-1', action=OrderAction.OPEN.value, reason='SELL selected', decision_id='decision-789', side=Side.SELL.value, volume=0.2, stop_loss=1.2, take_profit=1.19), timestamp_utc='2026-07-07T06:01:00.000Z')
