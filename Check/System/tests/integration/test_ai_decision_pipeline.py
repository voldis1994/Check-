...
from tests.integration.test_decision_pipeline import _parse_journal_lines, _startup_runtime, run_instance_decision_pipeline
...
def test_integration_pipeline_logs_ai_fields(tmp_path: Path) -> None:
    ...
    assert entry['system_decision_before_ai'] in {Decision.BUY.value, Decision.SELL.value, Decision.WAIT.value, Decision.BLOCK.value}
    assert entry['decision_after_ai'] == entry['system_decision_before_ai'] or entry['decision_after_ai'] in {Decision.BUY.value, Decision.SELL.value, Decision.WAIT.value, Decision.BLOCK.value}

@pytest.mark.no_ai_mock
def test_advisory_missing_api_key_falls_back_in_full_cycle(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    runtime, instance = _startup_runtime(tmp_path)
    result = run_instance_cycle(runtime, instance, use_global_universe=False, timestamp_utc='2026-07-07T06:02:00.000Z')
...
def test_ai_veto_before_risk_skips_allow_result(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    import engine.ai_decision_layer as mdl

    def _reject_all(*_args, **_kwargs) -> str:
        ai = AIDecision(bias='AVOID', confidence=0.9, allow_buy=False, allow_sell=False, allow_close=True, reason='mock avoid')
        return json.dumps({'bias': ai.bias, 'confidence': ai.confidence, 'allow_buy': ai.allow_buy, 'allow_sell': ai.allow_sell, 'allow_close': ai.allow_close, 'reason': ai.reason})
...
def test_risk_runs_after_ai_veto_aligns_journal(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    import engine.ai_decision_layer as mdl

    def _reject_all(*_args, **_kwargs) -> str:
        return json.dumps({'bias': 'AVOID', 'confidence': 0.9, 'allow_buy': False, 'allow_sell': False, 'allow_close': True, 'reason': 'integration risk order check'})
    monkeypatch.setattr(mdl, '_call_openai', _reject_all)
    runtime, instance = _startup_runtime(tmp_path)
    result = run_instance_decision_pipeline(runtime, instance, use_global_universe=False)
    assert result.completed
...
@pytest.mark.no_ai_mock
def test_required_mode_blocks_without_api_key(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
