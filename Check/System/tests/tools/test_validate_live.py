...
from tools.validate_live import LiveValidationReport, ValidationCheck, format_validation_report, main, run_live_validation, validate_config_file, validate_directory_layout, validate_instance_isolation, validate_mql4_ea_files, validate_mt4_exports, validate_rules_compliance, validate_runtime_entry_points, validate_universe_context
...
def _install_mt4_exports(paths: SystemPaths, instance: Instance) -> None:
    ...
    shutil.copyfile(INTEGRATION_FIXTURES / instance.market_filename(), account_dir / instance.market_filename())
    shutil.copyfile(INTEGRATION_FIXTURES / instance.sensor_filename(), account_dir / instance.sensor_filename())
    shutil.copyfile(INTEGRATION_FIXTURES / instance.status_filename(), account_dir / instance.status_filename())
...
def _prepare_live_root(tmp_path: Path, *, with_exports: bool=True) -> tuple[Path, SystemPaths, Instance]:
    ...
    paths = build_system_paths(__import__('engine.core.config', fromlist=['load_system_config']).load_system_config(config_path, system_paths=SystemPaths(tmp_path)))
...
def test_validation_check_dataclass_fields() -> None:
    check = ValidationCheck(check_id='sample', name='sample check', passed=True, message='ok')
    assert check.check_id == 'sample'
    assert check.passed is True

def test_live_validation_report_passed_and_failed_checks() -> None:
    report = LiveValidationReport(checks=(ValidationCheck('a', 'A', True, 'ok'), ValidationCheck('b', 'B', False, 'bad')))
...
def test_validate_mt4_exports_fails_without_exports(tmp_path: Path) -> None:
    _, paths, instance = _prepare_live_root(tmp_path, with_exports=False)
    checks = validate_mt4_exports(paths, (instance,), stale_threshold_ms=60000, now_epoch=time.time())
...
def test_validate_universe_context_rejects_trade_signal_fields(tmp_path: Path) -> None:
    ...
    paths.universe_file.write_text(json.dumps({'schema_version': PROTOCOL_SCHEMA_VERSION, 'timestamp_utc': '2026-07-07T06:00:00.000Z', 'session': 'LONDON', 'market_regime': 'trending', 'signal': 'BUY'}), encoding='utf-8')
...
def test_validate_rules_compliance_checks_code_and_config(tmp_path: Path) -> None:
    config_path, paths, _ = _prepare_live_root(tmp_path, with_exports=False)
    config = __import__('engine.core.config', fromlist=['load_system_config']).load_system_config(config_path, system_paths=paths)
    checks = validate_rules_compliance(paths.root, config, config_text=config_path.read_text(encoding='utf-8'))
    assert all((check.passed for check in checks))
...
def test_run_live_validation_can_skip_mt4_exports(tmp_path: Path) -> None:
    ...
    report = run_live_validation(root_path=tmp_path, config_path=config_path, require_mt4_exports=False)
    assert report.passed

def test_format_validation_report_includes_result_line() -> None:
    report = LiveValidationReport(checks=(ValidationCheck('a', 'A', True, 'ok'),))
