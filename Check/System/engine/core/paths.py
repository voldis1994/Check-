...
def _normalize_relative_path(value: str, field_name: str) -> str:
    if not isinstance(value, str):
        raise _validation_error(f'{field_name} must be a string', field=field_name, value_type=type(value).__name__)
    normalized = value.replace('\\', '/').strip()
    if not normalized:
        raise _validation_error(f'{field_name} must not be empty', field=field_name)
    if normalized.startswith('/') or ':' in normalized:
        raise _validation_error(f'{field_name} must be a relative path', field=field_name, value=value)
    return normalized
...
class SystemPaths:
    ...
    def __init__(self, root_path: str | Path | None=None, *, clients_path: str=DEFAULT_CLIENTS_PATH, logs_path: str=DEFAULT_LOGS_PATH, cache_path: str=DEFAULT_CACHE_PATH, history_path: str=DEFAULT_HISTORY_PATH, universe_path: str=DEFAULT_UNIVERSE_PATH) -> None:
    ...
    def instance_cache_dir(self, account_id: str, symbol: str, magic: int) -> Path:
        return self.cache_dir / validate_account_id(account_id, PATHS_MODULE) / _instance_segment(symbol, magic)

    def instance_history_dir(self, account_id: str, symbol: str, magic: int) -> Path:
        return self.history_dir / validate_account_id(account_id, PATHS_MODULE) / _instance_segment(symbol, magic)

    def ensure_directories(self) -> None:
        for directory in (self.clients_dir, self.logs_dir, self.cache_dir, self.history_dir, self.universe_dir):
            directory.mkdir(parents=True, exist_ok=True)

    def ensure_account_directories(self, account_id: str) -> None:
        self.ensure_directories()
        for directory in (self.account_dir(account_id), self.account_journal_dir(account_id), self.account_state_dir(account_id)):
            directory.mkdir(parents=True, exist_ok=True)

    def ensure_instance_directories(self, account_id: str, symbol: str, magic: int) -> None:
        self.ensure_account_directories(account_id)
        for directory in (self.instance_cache_dir(account_id, symbol, magic), self.instance_history_dir(account_id, symbol, magic)):
            directory.mkdir(parents=True, exist_ok=True)
