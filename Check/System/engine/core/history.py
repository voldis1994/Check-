...
def archive_processed_control(paths: SystemPaths, instance: Instance) -> Path | None:
    ...
    destination_dir = paths.instance_history_dir(instance.account_id, instance.symbol, instance.magic)
...
def archive_market_snapshot(paths: SystemPaths, instance: Instance, *, current_utc: str | None=None) -> Path | None:
...
def find_latest_archived_file(directory: Path, *, prefix: str) -> Path | None:
    if not directory.is_dir():
        return None
    matches = sorted((entry for entry in directory.iterdir() if entry.is_file() and entry.name.startswith(prefix)), key=lambda entry: entry.stat().st_mtime)
    if not matches:
        return None
    return matches[-1]

def read_archived_control_text(paths: SystemPaths, instance: Instance) -> str | None:
    history_dir = paths.instance_history_dir(instance.account_id, instance.symbol, instance.magic)
