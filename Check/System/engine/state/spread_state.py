...
class SpreadState:
    ...
    def update_from_snapshot(self, snapshot: SpreadModelSnapshot, updated_utc: str) -> SpreadStateRecord:
        self.record = SpreadStateRecord(schema_version=STATE_SCHEMA_VERSION, account_id=self.instance.account_id, symbol=self.instance.symbol, magic=self.instance.magic, sample_count=snapshot.sample_count, mean_spread=snapshot.mean_spread, std_spread=snapshot.std_spread, median_spread=snapshot.median_spread, current_spread=snapshot.current_spread, relative_spread=snapshot.relative_spread, updated_utc=updated_utc)
        return self.record

    def save(self, paths: SystemPaths) -> None:
        if self.record is None:
            raise ValidationError('spread state record is not initialized', module='state.spread_state')
