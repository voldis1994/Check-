...
class TrendAnalysis:
    ...
    def from_momentum(cls, momentum: MomentumAnalysis) -> TrendAnalysis:
        return cls(trend_direction=momentum.trend_direction, trend_strength=momentum.trend_strength, trend_duration_bars=momentum.trend_duration_bars, higher_highs=momentum.higher_highs, lower_lows=momentum.lower_lows)

def _clamp(value: float, min_value: float, max_value: float) -> float:
    return max(min_value, min(max_value, value))

def analyze_momentum_and_trend(bars: tuple[NormalizedMarketBar, ...]) -> MomentumAnalysis:
    if len(bars) < 2:
        return MomentumAnalysis(momentum_score=0.0, momentum_direction=MomentumDirection.NEUTRAL.value, rate_of_change=0.0, acceleration=0.0, trend_direction=TrendDirection.SIDEWAYS.value, trend_strength=0.0, trend_duration_bars=len(bars), higher_highs=False, lower_lows=False)
    ...
    return MomentumAnalysis(momentum_score=momentum_score, momentum_direction=momentum_direction, rate_of_change=rate_of_change, acceleration=acceleration, trend_direction=trend_direction, trend_strength=trend_strength, trend_duration_bars=len(bars), higher_highs=higher_highs, lower_lows=lower_lows)
