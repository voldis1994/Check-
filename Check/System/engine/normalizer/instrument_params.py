...
def derive_instrument_params(bars: Sequence[NormalizedMarketBar]) -> InstrumentParams:
    if not bars:
        raise ValidationError('market bars are required to derive instrument parameters', module='normalizer.instrument_params')
    first_bar = bars[0]
    return InstrumentParams(symbol=first_bar.symbol, digits=first_bar.digits, point=first_bar.point, pip=calculate_pip(first_bar.point, first_bar.digits))
