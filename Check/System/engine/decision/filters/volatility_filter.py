...
def calculate_relative_volatility(bars: Sequence[NormalizedMarketBar], *, lookback_bars: int) -> float:
    if lookback_bars <= 0:
        raise ValidationError('lookback_bars must be positive', module='decision.filters.volatility_filter', context={'lookback_bars': lookback_bars})
    if not bars:
        return 0.0
...
def evaluate_volatility_filter(relative_volatility: float, threshold: float) -> VolatilityFilterResult:
    volatility_acceptable = relative_volatility <= threshold
    reason: str | None = None
    if not volatility_acceptable:
        reason = build_reason(REASON_VOLATILITY_ABNORMAL, 'relative volatility above threshold', relative_volatility=relative_volatility, threshold=threshold)
    return VolatilityFilterResult(volatility_acceptable=volatility_acceptable, relative_volatility=relative_volatility, threshold=threshold, reason=reason)
