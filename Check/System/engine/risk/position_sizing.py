...
def compute_stop_loss_distance_points(*, entry_price: float, stop_loss: float, point: float) -> float:
    if point <= 0:
...
def compute_point_value_per_lot(*, point: float, units_per_lot: float) -> float:
    if point <= 0:
        raise _validation_error('point must be > 0', point=point)
    if units_per_lot <= 0:
...
def calculate_position_size(*, equity: float, max_risk_per_trade_percent: float, entry_price: float, stop_loss: float, point: float, pip: float, volume_step: float, units_per_lot: float=100000.0) -> PositionSizingResult:
    ...
    if equity <= 0:
        return PositionSizingResult(allowed=False, volume=0.0, reason=build_reason(REASON_INVALID_VOLUME, 'equity must be positive', equity=equity))
    if max_risk_per_trade_percent <= 0:
        return PositionSizingResult(allowed=False, volume=0.0, reason=build_reason(REASON_INVALID_VOLUME, 'max_risk_per_trade_percent must be positive', max_risk_per_trade_percent=max_risk_per_trade_percent))
    stop_loss_distance_points = compute_stop_loss_distance_points(entry_price=entry_price, stop_loss=stop_loss, point=point)
    if stop_loss_distance_points <= 0:
        return PositionSizingResult(allowed=False, volume=0.0, reason=build_reason(REASON_INVALID_VOLUME, 'stop loss distance must be positive', entry_price=entry_price, stop_loss=stop_loss, point=point, pip=pip))
