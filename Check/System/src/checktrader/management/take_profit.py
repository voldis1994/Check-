...
def take_profit_action(position: Position, bid: float, ask: float) -> ManagementAction:
    ...
    if position.side == Side.BUY and bid >= position.take_profit:
        return ManagementAction(Decision.CLOSE, ReasonCode.TAKE_PROFIT_HIT, OrderAction.CLOSE, close_fraction=1.0)
    if position.side == Side.SELL and ask <= position.take_profit:
        return ManagementAction(Decision.CLOSE, ReasonCode.TAKE_PROFIT_HIT, OrderAction.CLOSE, close_fraction=1.0)
    return ManagementAction(Decision.HOLD, ReasonCode.MANAGEMENT_NO_ACTION)
