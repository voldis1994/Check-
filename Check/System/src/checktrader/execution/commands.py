...
def _command(action: OrderAction, symbol: str, payload: dict[str, object]) -> Command:
    return Command(uuid4().hex, action, symbol, protocol_version, datetime.now(UTC), payload)
...
def build_open(signal: StrategySignal, lot: float, config: ExecutionConfig) -> Command:
    return _command(
        ...
        signal.symbol,
        {
            ...
            "reason": ReasonCode.ORDER_OPEN_BUILT.value,
        },
    )
...
def build_modify(position: Position, action: ManagementAction) -> Command:
    return _command(
        ...
        position.symbol,
        {
            ...
            "reason": ReasonCode.ORDER_MODIFY_BUILT.value,
        },
    )
...
def build_close(position: Position, action: ManagementAction) -> Command:
    return _command(
        ...
        position.symbol,
        {
            ...
            "reason": ReasonCode.ORDER_CLOSE_BUILT.value,
        },
    )
